...
| Database (PostgreSQL) | Persists tenant-owned configuration, repository metadata, analysis history, immutable completed-analysis facts, findings, metrics, prediction results, and non-authoritative profile-stamped snapshot score summaries. | Workspace and repository data, analysis attempts, snapshots, findings, metrics, prediction data, scoring configuration, derived score cache, model provenance, security-audit records. |
...
| Snapshot | Represents the immutable result of a successfully completed analysis. | Stores analysis facts only. Scores are derived for the active profile; a separately keyed SnapshotScore row may cache the summary without modifying the snapshot. |
...
A profile change updates only the active profile and writes no snapshot. After commit, it queues score-cache warm-up tasks for existing immutable snapshots (SRS FR-20, FR-21).
