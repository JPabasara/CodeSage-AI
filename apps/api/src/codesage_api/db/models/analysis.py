...
class AnalysisAttempt(UUIDPrimaryKey, Base):
    ...
    analysis_engine_version_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("analysis_engine_version.id", ondelete="RESTRICT"), index=True)
    commit_sha: Mapped[str] = mapped_column(
        String(64), nullable=False)
    trigger_type: Mapped[AnalysisTriggerType] = mapped_column(
        Enum(AnalysisTriggerType, name="analysis_trigger_type", values_callable=values))
    status: Mapped[AnalysisStatus] = mapped_column(
        Enum(AnalysisStatus, name="analysis_status", values_callable=values), index=True)
