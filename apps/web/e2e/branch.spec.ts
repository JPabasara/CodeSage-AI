...
test("the branch defaults to the repository's default branch", async ({ page }) => {
  // `main` is the only branch with `is_default: true`; nothing was picked yet.
  await expect(page.getByRole("combobox", { name: /branch/i })).toContainText("main")
})

test("a branch with no head commit renders without printing null", async ({ page }) => {
...
test("switching back to main restores the original numbers", async ({ page }) => {
  await pickBranch(page, "develop")
  await expect(page.getByText("66/100")).toBeVisible()
